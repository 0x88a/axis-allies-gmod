...
local send_syntax = function(ply, args, cmd_name, cmd_args, cmd_args_n, from_console)
	...
	ply:sam_add_text(unpack(tbl))
	return ""
end

local run_command = function(ply, text, from_console, silent)
	local args = sam.parse_args(text)
	local cmd_name = lower(args[1])
	...
	for i = 1, cmd_args_n do
		...
		if arguments[cmd_arg.name](cmd_arg, arg, ply, cmd, result) == false then
			return ""
		end
	end

	sam.is_command_silent = silent
	...
	hook.Call("SAM.RanCommand", nil, ply, cmd_name, args, cmd)
...
hook.Add("PlayerSay", "SAM.Command.RunCommand", function(ply, text)
	...
	local silent = prefix == "~" and ply:HasPermission("silent_commands")
	return run_command(ply, sub(text, 2), false, silent)
end)
