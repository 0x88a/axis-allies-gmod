...
local lang = sam.config.Language
...
if SERVER then
	AddCSLuaFile(lang_path)
end
local Language = include(lang_path)
...
do
	...
	function sam.format_message(msg, tbl, result, result_n)
		...
		while true do
			...
			args[sub(arg, 1, 1)](result, tbl and tbl[arg], arg, unpack(arg2:Trim():Split(" ")))
		...
		if sam.isconsole(admin) then
			...
			if sam.type(admin) == "Player" then
				if CLIENT and LocalPlayer() == admin then
					insert(result, Color(250, 159, 66))
