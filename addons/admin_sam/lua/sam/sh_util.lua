...
function sam.get_targets_list(targets)
	...
	if len == player.GetCount() and len > 1 then
		return {Color(244, 67, 54), "everyone"}
	end
	...
	for i = 1, len do
		...
		if CLIENT and LocalPlayer() == target then
			table.insert(result, Color(250, 159, 66))
...
function sam.is_steamid64(id, err)
	if sam.isstring(id) and id:sub(1, 7) == "7656119" and (#id == 17 or #id == 18) then
		return true
	else
...
function sam.hook_first(event, name, func)
	if DLib then
		return hook.Add(event, name, func, HOOK_MONITOR_HIGH)
	end

	local event_hooks = hook.GetTable()[event] or {}

	for hook_name in pairs(event_hooks) do
	...
	for hook_name, hook_func in pairs(event_hooks) do
		if hook_name ~= name then
			hook.Add(event, hook_name, hook_func)
		end
	end
end