...
do
	local prefix_result = sam.format_message(sam.isstring(sam.config.ChatPrefix) and sam.config.ChatPrefix or "")
...
do
	...
	function PLAYER:sam_get_play_time()
		return self:sam_get_nwvar("play_time") + self:sam_get_session_time()
	end

	function PLAYER:sam_get_session_time()
		return os.time() - self:sam_get_nwvar("join_time")
	end
