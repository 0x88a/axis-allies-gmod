...
do
	...
	hook.Add("Think", "SAM.Player.SaveTimes", function()
		...
		SQL.Begin()
		local players, args = player.GetHumans(), {}
		for i = 1, #players do
			local ply  = players[i]
			if ply:sam_get_nwvar("is_authed") then
				args[1] = math.floor(ply:sam_get_play_time())
				args[2] = ply:SteamID()
	...
	hook.Add("PlayerDisconnected", "SAM.Player.SaveTime", function(ply)
		if ply:sam_get_nwvar("is_authed") then
			SQL.FQuery([[
				UPDATE
					`sam_players`
				...
				WHERE
					`steamid` = {2}
			]], {math.floor(ply:sam_get_play_time()), ply:SteamID()})
		end
	end)
end