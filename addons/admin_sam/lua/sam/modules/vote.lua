...
if CLIENT then
	...
	surface.CreateFont("SAM.VotingTitle", {
		font = "Roboto Bold",
	...
	surface.CreateFont("SAM.VotingOption", {
		font = "Roboto Medium",
	...
	hook.Add("SAM.ChangedGlobalVar", "Voting", function(key, value)
		...
		voting_frame:SetSize(sam.menu.scale(165), sam.menu.scale(230))
		voting_frame:SetPos(5, ScrH() * 0.25)
		voting_frame:DockPadding(4, 4, 4, 4)
		voting_frame:Blur()
			:Background(Color(30, 30, 30, 240))

		local voting_title = voting_frame:Add("DLabel")
		voting_title:Dock(TOP)
		voting_title:SetFont("SAM.VotingTitle")
		voting_title:SetTextColor(Color(220, 220, 220))
		voting_title:SetText(title)
		voting_title:SetWrap(true)
		voting_title:SetAutoStretchVertical(true)

		local line = voting_frame:Add("DLabel")
		line:Dock(TOP)
		line:SetTextColor(Color(220, 220, 220))
		line:SetText("-")

		local options_added = {}
		for i, v in ipairs(options) do
			local option = voting_frame:Add("DLabel")
			option:Dock(TOP)
			option:SetFont("SAM.VotingOption")
			option:SetTextColor(Color(220, 220, 220))
		...
		hook.Add("PlayerBindPress", "SAM.Voting", function(_, bind, down)
			...
			if current_index then
				options_added[current_index]:SetTextColor(Color(220, 220, 220))
			end

			options_added[index]:SetTextColor(Color(65, 185, 255))
			current_index = index
...
command.new("vote")
	...
	:OnExecute(function(ply, title, ...)
		local callback = function(_, option, count, total_count)
			sam.player.send_message(nil, "Vote for {V} has been passed. ({V_2}/{V_3})", {
