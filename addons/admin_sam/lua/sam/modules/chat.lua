...
command.new("pm")
	...
	:Help(language.get("pm_help"))

	:OnExecute(function(ply, targets, message)
		if ply:sam_get_nwvar("muted") then
...
do
	...
	local reports_enabled = sam.config.Reports.Enabled
	command.new("asay")
		...
		:Help(language.get("asay_help"))

		:OnExecute(function(ply, message)
			if reports_enabled and not ply:HasPermission("see_admin_chat") then
...
do
	...
	if SERVER then
		sam.hook_first("PlayerSay", "SAM.Chat.Mute", function(ply, text)
			...
			if text:sub(1, 1) == "!" and text:sub(2, 2):match("%S") ~= nil then return end
