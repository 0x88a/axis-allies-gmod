...
if true then
	...
	function F4menu.Open()
		...
		close:SetSize(52, 32)
		close:SetPos(F4menu.frame:GetWide()-64-25, 50+close:GetTall())
		close:SetText("")
		local size = 12
		local alpha = 25
		local delay = 0.5
		...
		function close:Paint(w, h)
			...
			if self.Hovered then
				size = math.Approach(size, 30, FrameTime()*10)
			...
			surface.SetDrawColor(206, 55, 55, 255)
			draw.NoTexture()
			surface.DrawRect(0, 0, w, h)
			surface.DrawOutlinedRect(0, 0, w, h)
			    ...
			    tabs.Add("Jobs", F4menu.configuration.tabs.jobs.color, function(p)
			    	...
			    	for k, v in pairs(DarkRP.getCategories()["jobs"]) do
			    		...
			    		function category:Paint(w, h)
			    			local h = 48
							local teamCol = team.GetColor(count)
			    			surface.SetDrawColor(theme().listing_header)
			    			surface.DrawRect(0, 0, w, h)

			    			surface.SetDrawColor(teamCol) --  niggers
